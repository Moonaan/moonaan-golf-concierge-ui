...
export function useAgentConnection() {
  ...
  useEffect(() => {
    ...
    conn.on('textResponse', (event: AgentEvent) => {
      if (event.type === 'textResponse') {
        setMessages((prev) => [...prev, { id: `agent_${Date.now()}`, role: 'agent', text: event.text, timestamp: new Date() }]);
      }
    });

    conn.on('transcript', (event: AgentEvent) => {
      if (event.type === 'transcript' && event.role === 'agent' && event.isFinal) {
        setMessages((prev) => [...prev, { id: `agent_tr_${Date.now()}`, role: 'agent', text: event.text, timestamp: new Date() }]);
      }
    });
  ...
  const sendText = useCallback((text: string) => {
    ...
    setMessages((prev) => [...prev, { id: `user_${Date.now()}`, role: 'user', text, timestamp: new Date() }]);
  }, []);
